...
avatar1 ="https://bit.ly/3cyaPF9"
avatar2 ="https://bit.ly/3x6M1xF"
...
def main():
    ...
    if choice == "Home":
        ...
        img = Image.open("image/hepatitis-b-virus.png")
        ...
        if st.sidebar.checkbox("Login"):
            ...
            if result:
                ...
                if activity == "Plot":
                    ...
                    # df['class'].value_counts().plot(kind='bar', color ='indigo')
                    # st.pyplot(fig)

                    # Freq_dis plot

                    freq_df = pd.read_csv('data/freq_hepatitis_dataset.csv')
                ...
                # PREDICTION
                    ...
                    # Model Choice
                    model_choice = st.selectbox("Select Model", ["Logistic Regression", "K-Nearest Neighbors", "Decision Tree"])
                    if st.button('Predict'):
                        ...
                        # st.write(prediction)
                        # prediction_label = {"Low": 1, "High": 2}
                        # final_result = get_key(prediction, prediction_label)   
                        if prediction == 0.6 and prediction > 0.6:
                            st.warning("Patient Chances of Survival is Low")
                            ...
                            st.success("Patient Chances of Survival is High")
                            pred_probability_score = {"Low":pred_prob[0][0]*100,"High":pred_prob[0][1]*100} 
                            st.subheader("Predicton Probability Score using {}".format(model_choice))
                            st.json(pred_probability_score)
                            st.markdown(prescriptive_message_temp,unsafe_allow_html=True)
